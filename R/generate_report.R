generate_report <- function(presence_table, elements = c("summary_table", "count_table", 
                                                         "summary_plot", "presence_plot"),
  ...
  # Generate tables and plots which are given in 'elements' argument
  if("summary_table" %in% elements) {
    summary_table <- get_summary_table(presence_table, hide_absent = hide_absent_systems)
    write.csv(summary_table, paste0(outdir, "/summary_table.csv"))
  }
  ...
  if("count_table" %in% elements) {
    ...
    write.csv(count_table, paste0(outdir, "/count_table.csv"))
  }
  ...
  rmarkdown::render(report_template, output_format = "html_document", 
                    ...
                    params = list(genome_files, outdir))

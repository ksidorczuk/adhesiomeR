...
#' @export
get_adhesin_profiles <- function(new_results, model_all, model_fimbrial, model_nonfimbrial) {
  do.call(rbind, 
          lapply(1:nrow(new_results), function(i) {
            ...
            data.frame(File = new_results[i,][["File"]],
                       Adhesins_profile = get_cluster(model_all, new_x[which(colnames(new_x) %in% colnames(model_all[["data"]]))]),
                       Fimbrial_profile = get_cluster(model_fimbrial, new_x[which(colnames(new_x) %in% colnames(model_fimbrial[["data"]]))]),
                       Nonfimbrial_profile = get_cluster(model_nonfimbrial, new_x[which(colnames(new_x) %in% colnames(model_nonfimbrial[["data"]]))]))
          })) 
}
